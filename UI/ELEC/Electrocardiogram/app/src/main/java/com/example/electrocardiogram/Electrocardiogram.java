package com.example.electrocardiogram;
...
public class Electrocardiogram extends View {
    ...
    private void init() {
        ...
        backgroundColor = Color.parseColor("#ffe0f6f6");
...
//        canvas.drawPath(electrocarPath, electrocarPaint);
    ...
    public void generateElectrocar() {
        ...
        for (int i = 0; i < 25; i++) {         // 前面 12 个小格
            datas.add(0f);                        // 为平滑水平的直线
        }
        for (int i = 0; i < 63; i++) {            // 这里开始陡峭起来
            double random;
            if (i % 2 == 0) {                        // 一个为正数
                random = Math.random();
            } else {                                // 另一个为负数
    ...
    private void drawGird(Canvas canvas) {
        paint.setColor(Color.WHITE);                   // 网格浅绿
        // 横向的网格
