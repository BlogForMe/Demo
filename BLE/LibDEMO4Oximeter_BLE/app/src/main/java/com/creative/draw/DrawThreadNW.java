		...
		synchronized (this) {
			while (!stop) {
				try {
					...
					if (MainActivity.SPO_WAVE.size() > 0) {
						Wave data = MainActivity.SPO_WAVE.remove(0);
						addData(data.data);
						if (data.flag == 1) { //发送搏动标记
							mHandler.sendEmptyMessage(MainActivity.MSG_DATA_PULSE);
						}
						
						//设置参数，调整波形，adjust wave
						if (MainActivity.SPO_WAVE.size() > 20) {
							Thread.sleep(12); //18
						} else {
							Thread.sleep(25); //25
						}							
